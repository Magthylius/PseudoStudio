...
public class RobotMovementController : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		...
		bool noNewPath = newPath == null || !newPath.isCalculating;
		bool needToRebuild = Vector3.SqrMagnitude(target.transform.position - lastDestination) > maxDistanceRebuildPath;
		bool playerOutOfSight = !CanSeePlayer() || Vector3.Distance(target.position,transform.position) > minFollowDistance;
		bool octreeNotBuilding = !octree.IsBuilding;

		if (noNewPath && needToRebuild && playerOutOfSight && octreeNotBuilding)
		{
