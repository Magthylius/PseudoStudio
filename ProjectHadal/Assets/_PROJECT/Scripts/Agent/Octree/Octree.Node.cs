...
namespace FlyAgent.Navigation
{
	public partial class Octree
	{
		public class Node : System.IComparable<Node>, IPFVertex<Node>, System.IEquatable<Node>
		{
			...
			/// <summary> The center point of the boundary box of the node. </summary>
			...
			/// <summary> Actual length of sides, taking the looseness value into account </summary>
			float m_AdjLength;

			/// <summary> Reference of all Child nodes, if any </summary>
			private Node[] m_Children = null;

			/// <summary> Bounds of potential children to this node. These are actual size (with looseness taken into account), not base size </summary>
			private Bounds[] m_ChildBoundsCache;

			/// <summary> Returns a list of bounds perfectly matching the <see cref="eNeighbor"/> order</summary>
			...
			/// <summary> The octree data structure reference. </summary>
			public readonly Octree m_Root;

			/// <summary> Reference to the parent of this node if it is a child to any </summary>
			public Node m_Parent { get; protected set; }

			/// <summary> Periodically cached neighbours of this node. </summary>
			private List<Node>[] m_NeighborsCache = null; // 0~25 + 26
			private Vector3[] m_Vertices = null;

			/// <summary> List of Objects (obstacles) in this node, a get/set wrapper for m_Data </summary>
            ...
            /// <returns> Note: I am not sure if this is in seconds or milliseconds lol </returns>
			...
			/// <param name="parent">This will be the new parent</param>
			...
			/// <summary>
            /// Returns all the children of this node (min: Empty Node, max: 8)
            /// </summary>
			...
			/// <summary>
            /// Check if this node contains the specified obstacle
            /// </summary>
			...
			/// <summary>
			/// Returns an array of objects that intersect with the specified bounds, if any. Otherwise returns an empty array.
			/// </summary>
			...
			/// <summary>
			/// Returns an array of objects that intersect with the specified Unity ray, if any. Otherwise returns an empty array. See also: IsColliding.
			/// </summary>
			...
			/// <summary>Get all neighbors which are fully colliding with the anchor node</summary>
			/// <param name="neighbors">list of neighbors</param>
			/// <param name="checkBounds">reference bounds area.</param>
			/// <param name="anchor">reference object</param>
			...
			/// <summary>Find a child node that fits the best in the bounds. It assumes that the parent fully 
            /// encapsulates <paramref name="checkBounds"/>.</summary>
			...
			/// <summary>
            /// Note: this is a recursive composite function
            /// <br/>
            /// Returns a list of all neighbours that are colliding with all children below this node's level of depth
            /// </summary>
			...
			/// <summary>
            /// Returns an array of vertices that point in the direction specified by <see cref="eNeighbor"/>.
            /// </summary>
			...
			/// <summary>
            /// Returns a list of confirmed neighbours of this node from the cache (<see cref="m_NeighborsCache"/>). If the cache 
            /// is empty, search the ENTIRE tree for neighbours. If there is something in the cache, only search for neighbours
            /// from the parent of this node onwards.
            /// </summary>
			...
			/// <summary>Searches for neighbours of this node by checking the entire tree.</summary>
			...
			/// <summary>
            /// Checks the parent's neighbours + childrens (except this node) if they are eligible neighbours for this node </summary>
			/// <param name="extendBounds"></param>
			...
			/// <summary>
            /// Checks to see if any nodes in <paramref name="data"/> are potential neighbours of this node. If so, cache that neighbour
            /// in <see cref="m_NeighborsCache"/>.
            /// </summary>
			...
			/// <summary>
            /// Note: this is a recursive^2 composite^2 function.
            /// <br/>
            /// Checks if neighbours in <see cref="m_NeighborsCache"/> are eligible and will add/remove any based on collision criteria
            /// </summary>
			internal void UpdateNeighborsIfNeed()
			{
				...
				if (m_NeighborsCache == null)
					return; // on demand, avoid common case the edge node never used at runtime.
            ...
            /// Note: this is a recursive composite function.
            /// <br/>
            /// Removes a single node reference from all <see cref="m_NeighborsCache"/> from this node and nodes in the neighbours cache.
            /// </summary>
			...
			/// Must be called from OnDrawGizmos externally. <see cref="DrawAllBounds"/>
			/// </summary>
			...
			/// <br/>- This node is >= double of <paramref name="minLength"/> in length
			/// <br/>- All objects in the root node are within one octant
			/// <br/>- This node doesn't have children, or does but 7/8 children are empty
            ...
            /// Note: this is a recursive composite function.
            /// <br/>
            /// Adds or removes obstacles based on collision criteria
            /// </summary>
            /// <param name="obstacle"></param>
            ...
            /// <br/>
            /// 1) Add obstacle <br/>
            /// 2) Detect obstacle edge within this node <br/>
            /// 3) Pass the obstacle recursively to the children <br/>
            /// </summary>
			/// <param name="obstacle">obstacle to add</param>
			/// <param name="continueAdd">this add request comes from <see cref="ContinueInternalSplit"/></param>
			...
			/// <summary>
            /// Requests that a node will be split to be remerged elsewhere. This request should only based on obstacles within the nodes, 
            /// since the purpose of this octree is for pathfinding.
            /// <br/>
            /// Jet (I think this is what it means): If an obstacle is within the bounds of less than 4 or 5 children nodes, therefore, 
            /// the obstacle detection can be more refined by splitting into smaller pieces.
            /// <br/>
            /// If the current <paramref name="m_SpiltQuota"/> is not sufficient to execute the split request, wait for the next
            /// update frame.
            /// </summary>
			/// <param name="obstacle"></param>
			...
			/// <summary>
            /// Since the Octree update operates on async, the node must be ready to handle the split at any given time in
            /// the game. This function is an async callback for <see cref="InternalSplitRequest"/>, and <seealso cref="ContinueObstacleSplitRequest"/>.
			/// When there is not enough quota, the split request will be delayed and tried again in the next update frame.
            /// </summary>
            ...
            /// Note: this is a recursive composite function.
            /// <br/>
            /// Removes obstacle reference from this node
            /// </summary>
			...
			/// <summary>
            /// Caches subdivision boundaries for the 8 possible children to fit in. Looseness more than 1f will add whitespace
            /// between these 8 subdivisions.
            /// </summary>
			...
			/// <summary>
			/// Sends a merge request up to the octree root queue. It will then wait for a response.
			/// <br/>Logic:
			/// If previous request is still cooling down, the duration for the lock will double;
			/// When previous request has expired, the duration will divide by 2.
			/// <br/>Result:
			/// If the node is sending merge requests too often, it will be rejected here.
			/// </summary>
			...
			/// <summary>
            /// Checks if children nodes can be merged into this node. This check should only apply to the depth level (N-1).
            /// </summary>
			/// <returns>true = all children are leaf nodes.</returns>
			bool ShouldMerge()
			{
			...
			/// <summary>
			/// Merges all children into this node - the opposite of Split.
            /// <br/>
			/// Note: We only have to check one level down since a merge will never happen if the children already has children
            /// (can only merge if all children are leaf nodes).
            /// <br/>
            /// Jet: Therefore, not a recursive composite function :D
			/// </summary>
			void Merge()
			{
			...
			/// <summary>Checks if this node is leaf Node.</summary>
			...
			/// <summary>
            /// Note: This is a recursive composite function.
            /// Checks if this node or anything below it has an obstacle in it.
            /// </summary>
			/// <returns>True if this node or any of its children, grandchildren etc have obstacle(s) in them</returns>
			...
			/// <summary>
            /// Note: This is a recursive composite function.
            /// Should only be called from root node, to calculate the max depth in tree.
            /// </summary>
			...
			/// <summary> Just returns a hash identifier </summary>
			...
			/// <summary> Overriding the .ToString() method </summary>
			...
			/// <summary>
            /// CompareTo is used by the priority queue <see cref="NetworkGraph.m_PQueue"/>, to determine which nodes in the queue
            /// to dequeue first when performing code logic. 
            /// <br/>
            /// For priority queue usage, the size is compared, instead of position.
            /// In this case, the BIGGER the size, the higher priority the node has.
            /// </summary>
			...
			#region IPVertex
			/// <summary>
            /// Checks if this node has direct line of sight (or crow flies sight) of the <paramref name="other"/> node.
            /// </summary>
			...
			/// <summary>
            /// Calculates the cost for this node to travel to the <paramref name="other"/> node, in respect to the boundary sizes.
            /// The value will be infinite cost if the node is completely covered by an obstacle.
            /// </summary>
			...
			/// <summary>
            /// Returns an IEnumerable of neighbours to this node. If this check is run for the first time, there is no neighbour
            /// cache to return, therefore many recursive composite functions will run in the background to generate the cache.
            /// </summary>
			...
			/// <summary>
            /// When doing node comparison (i.e. NodeA == NodeB), it will only equate with the node bounds, as all nodes in the octree
            /// should have unique bounds of different scales.
            /// </summary>
